	...
	*/
	...
	for (i=(is_set("UPDATE"))?S_BRAND:CALL_CENTER; (pT = getSimpleTdefsByNumber(i)); i++)
	{
      ...
      pF = getTdefFunctionsByNumber(i);
		...
		{
			/* Special handling for returns tables when directly requested */
			if (tabid == i && (i == WEB_RETURNS || i == CATALOG_RETURNS || i == STORE_RETURNS))
			{
				/* Skip the child table - it will be handled when we reach the parent */
				continue;
			}
			else
			{
		...
		/* Map returns tables to their parent tables for generation */
		int actual_table_id = i;
		if (tabid == WEB_RETURNS && i == WEB_SALES)
			actual_table_id = WEB_RETURNS;
		else if (tabid == CATALOG_RETURNS && i == CATALOG_SALES)
			actual_table_id = CATALOG_RETURNS;
		else if (tabid == STORE_RETURNS && i == STORE_SALES)
			actual_table_id = STORE_RETURNS;
		
		if ((tabid != -1) && (i != tabid) && (actual_table_id != tabid))
	...
	if ((pT->flags & FL_SOURCE_DDL) && (is_set("UPDATE") == 0))
	{
		 ...
		 */
		 /* Handle returns table mapping - when returns is requested, generate parent */
		 int generation_table = i;
		 if (tabid == WEB_RETURNS && i == WEB_RETURNS)
		     generation_table = WEB_SALES;
		 else if (tabid == CATALOG_RETURNS && i == CATALOG_RETURNS)
		     generation_table = CATALOG_SALES;
		 else if (tabid == STORE_RETURNS && i == STORE_RETURNS)
		     generation_table = STORE_SALES;
		     
         split_work(generation_table, &kFirstRow, &kRowCount);
         /*
         * if there are rows to skip then skip them 
         */
         if (kFirstRow != 1)
         {
            row_skip(generation_table, (int)(kFirstRow - 1));
            pT = getSimpleTdefsByNumber(generation_table); /* Update pT for parent table */
            if (pT->flags & FL_PARENT)
         ...
         */
         gen_tbl(generation_table, kFirstRow, kRowCount);
		}
	}
